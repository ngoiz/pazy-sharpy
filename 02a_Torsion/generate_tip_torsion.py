...
def run_coupled_torsion_simulation(case_id, tip_load, skin_on, case_root='./cases/', output_folder='./output/'):
    ...
    pazy_settings = {'skin_on': skin_on,
                     ...
                     'num_elem': 2}
    case_name = 'pazy_torsion_{}'.format(case_id)
    ...
    # Lumped mass located 80mm ahead of the leading edge
    tors_mass_b = pazy.get_ea_reference_line() * 0.1 + 80e-3

    pazy.structure.add_lumped_mass((tip_load, pazy.structure.n_node//2, np.zeros((3, 3)),
                                    np.array([0, tors_mass_b, 0])))
